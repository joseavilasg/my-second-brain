...
export const getStaticProps = async () => {
	...
	const hostUrl = vercelUrl ? `https://${vercelUrl}` : "http://localhost:3000"
	const props = {}
	...
	let res = await axios.get(`${hostUrl}/api/fetch-link-preview?url=${props[communicationAdapters].url}`)
	props[communicationAdapters] = res.data

	res = await axios.get(`${hostUrl}/api/fetch-link-preview?url=${props[hexagonalExplained].url}`)
	props[hexagonalExplained] = res.data

	res = await axios.get(`${hostUrl}/api/fetch-link-preview?url=${props[dossierAndreas].url}`)
	props[dossierAndreas] = res.data

	res = await axios.get(`${hostUrl}/api/fetch-link-preview?url=${props[valueObjects].url}`)
	props[valueObjects] = res.data

	res = await axios.get(`${hostUrl}/api/fetch-link-preview?url=${props[understandingDtoPattern].url}`)
